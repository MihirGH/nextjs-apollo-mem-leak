...
const coursesNodeDetail = {
  ...
  description:
    "Find out in-depth information about programs that you will love",
};
...
const resolvers = {
  Query: {
    layout: async () => {
      await wait(Math.floor(500 + Math.random() * 500));
      return layout;
    },

    nodeDetail: async (_, variables) => {
      await wait(Math.floor(500 + Math.random() * 500));
      return variables.id === "clubs" ? clubsNodeDetail : coursesNodeDetail;
    },
  },
};
